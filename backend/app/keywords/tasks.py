...
async def process_resource_item_content(
    resource_item_obj, keywords_to_found, error, content
):
    if error:
...
async def process_resource(resource_obj, sem, client):
    ...
    if error:
        ...
        for el in data:
            try:
                resource_item = await ResourceItem.create(
                    resource_id=resource_obj.id, url=el
                )
                if el in [resource_url, f"{resource_url}/"]:
                    ...
                    await process_resource_item_content(
                        resource_item, task.keywords, None, content
                    )
            except Exception as e:
                LOG.error(e)
    ...
    if (
        resource_obj.done_https
        and resource_obj.done_http
        and resource_obj.error_https
        and resource_obj.error_http
    ):
        await ResourceItem.create(
            resource_id=resource_obj.id, url=resource_url, done=True
        )
        resource_obj.done = True
    ...
    async with aiohttp.ClientSession(
        ...
        while True:
            ...
            resources_qs = Resource.filter(
                ...
                | Q(
                    Q(done_https=True),
                    Q(error_https__isnull=False),
                    Q(done_http=False),
                    join_type="AND",
                )
                | Q(
                    Q(done_http=True),
                    Q(error_http__isnull=False),
                    Q(done_https=False),
                    join_type="AND",
                )
            ).limit(500)
